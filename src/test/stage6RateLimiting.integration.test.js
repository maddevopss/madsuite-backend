...
describe("Stage 6 PR G - Rate Limiting & Abuse Prevention", () => {
  ...
  describe("Summary Views", () => {
    ...
    test("Should get abuse detection summary for all organizations", async () => {
      const result = await rateLimitingAbuseService.getAbuseDetectionSummary();

      expect(result.summary).toBeDefined();
      expect(Array.isArray(result.summary)).toBe(true);
    });

    test("Should get traffic anomaly summary", async () => {
      const result = await rateLimitingAbuseService.getTrafficAnomalySummary(organizationId);

      expect(result.summary).toBeDefined();
      expect(Array.isArray(result.summary)).toBe(true);
    });

    test("Should get traffic anomaly summary for all organizations", async () => {
      const result = await rateLimitingAbuseService.getTrafficAnomalySummary();
  ...
  describe("Error Handling", () => {
    test("Should handle database errors gracefully in createRateLimitPolicy", async () => {
      const result = await rateLimitingAbuseService.createRateLimitPolicy(
        organizationId,
        null,
        {}
    ...
    test("Should handle errors in checkRateLimit", async () => {
      const result = await rateLimitingAbuseService.checkRateLimit(
        "invalid-uuid",
        "user",
        "key",
    ...
    test("Should return policy_not_found when policy missing", async () => {
      const result = await rateLimitingAbuseService.checkRateLimit(
        "00000000-0000-0000-0000-000000000000",
        "user",
        "key",
    ...
    test("Should handle errors in recordAbuseAlert", async () => {
      const result = await rateLimitingAbuseService.recordAbuseAlert(
        organizationId,
        null,
        {}
    ...
    test("Should handle errors in blockEntity", async () => {
      const result = await rateLimitingAbuseService.blockEntity(
        null,
        organizationId,
        "test",
    ...
    test("Should handle errors in addIpAccessControl", async () => {
      const result = await rateLimitingAbuseService.addIpAccessControl(
        organizationId,
        null,
        "allowlist",
    ...
    test("Should handle errors in detectTrafficAnomaly", async () => {
      const result = await rateLimitingAbuseService.detectTrafficAnomaly(
        organizationId,
        null
      );
    ...
    test("Should handle errors in recordBotDetection", async () => {
      const result = await rateLimitingAbuseService.recordBotDetection(
        organizationId,
        null
      );
    ...
    test("Should handle errors in queueRequest", async () => {
      const result = await rateLimitingAbuseService.queueRequest(
        organizationId,
        null,
        null,
  ...
  describe("Integration Scenarios", () => {
    test("Should detect brute force and block IP", async () => {
      const alert = await rateLimitingAbuseService.recordAbuseAlert(
        organizationId,
        "brute_force",
        {
      ...
      const block = await rateLimitingAbuseService.blockEntity(
        alert.alert_id,
        organizationId,
      ...
      const ipBlock = await rateLimitingAbuseService.addIpAccessControl(
        organizationId,
        "192.168.2.50",
        "blocklist",
        {
      ...
      expect(ipBlock.added).toBe(true);
    });

    test("Should detect DDoS and queue requests", async () => {
      const anomaly = await rateLimitingAbuseService.detectTrafficAnomaly(
        organizationId,
        {
          anomalyType: "distributed_attack",
          severityLevel: "critical",
      ...
      const queue = await rateLimitingAbuseService.queueRequest(
        organizationId,
        "user_ddos",
        "key_ddos",
        "192.168.3.1",
    ...
    test("Should track bot and rate limit", async () => {
      const bot = await rateLimitingAbuseService.recordBotDetection(
        organizationId,
        {
          botType: "scraper",
      ...
      expect(bot.recorded).toBe(true);

      const control = await rateLimitingAbuseService.addIpAccessControl(
        organizationId,
        "203.0.113.150",
        "blocklist",
        {
          reason: "Scraper bot detected",
          isPermanent: true
        }
      );

      expect(control.added).toBe(true);
    });

    test("Should combine rate limits and abuse detection", async () => {
      const policy = await rateLimitingAbuseService.createRateLimitPolicy(
        organizationId,
        "detection_policy",
        {
          policyType: "ip",
      ...
      const alert = await rateLimitingAbuseService.recordAbuseAlert(
        ...
        "api_abuse",
        {
          ...
          detectedBehavior: "API flooding"
        }
      );

      expect(alert.recorded).toBe(true);

      const summary = await rateLimitingAbuseService.getRateLimitSummary(organizationId);
