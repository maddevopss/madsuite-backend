...
describe('institutional resilience transaction policies', () => {
  test('accepts a complete event', async () => {
    const result = await evaluateInstitutionalResiliencePolicy('resilience.event.open', {
  ...
  test('rejects an event without evidence', async () => {
    const result = await evaluateInstitutionalResiliencePolicy('resilience.event.open', {
  ...
  test('requires a complete crisis cell', async () => {
    const result = await evaluateInstitutionalResiliencePolicy('resilience.crisis.activate', {
      idempotencyKey: 'cell-1', crisisCell: { eventId: 3, leadUserId: 2, mandate: 'Coordonner', proofReference: 'proof://cell/1' },
    });
    expect(result.allowed).toBe(true);
  });

  test('rejects a future decision', async () => {
    const result = await evaluateInstitutionalResiliencePolicy('resilience.decision.record', {
  ...
  test('requires independent communication approval', async () => {
    const result = await evaluateInstitutionalResiliencePolicy('resilience.communication.publish', {
  ...
  test('requires an exercise report', async () => {
    const result = await evaluateInstitutionalResiliencePolicy('resilience.exercise.complete', {
  ...
  test('requires lesson traceability', async () => {
    const result = await evaluateInstitutionalResiliencePolicy('resilience.lesson.record', {
  ...
  test('requires closure proof', async () => {
    const result = await evaluateInstitutionalResiliencePolicy('resilience.improvement.close', {
