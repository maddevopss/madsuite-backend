const { evaluatePolicy, registerPolicy } = require("./transaction-engine.service");

const QUALITY_PLAN_POLICY = "quality.plan.create@1";
const QUALITY_PLAN_TRANSITION_POLICY = "quality.plan.transition@1";
const INSPECTION_POLICY = "quality.inspection.record@1";
const NONCONFORMITY_TRANSITION_POLICY = "quality.nonconformity.transition@1";
const CORRECTIVE_ACTION_TRANSITION_POLICY = "quality.corrective_action.transition@1";
const AUDIT_TRANSITION_POLICY = "quality.audit.transition@1";
...
registerPolicy("quality.plan.create", "1", ({ input, idempotencyKey }) => {
...
registerPolicy("quality.plan.transition", "1", ({ input, idempotencyKey }) => {
  if (!validIdempotency(idempotencyKey)) return { allowed: false, code: "quality.idempotency_required" };
...
registerPolicy("quality.inspection.record", "1", ({ input, idempotencyKey }) => {
  if (!validIdempotency(idempotencyKey)) return { allowed: false, code: "quality.idempotency_required" };
...
registerPolicy("quality.nonconformity.transition", "1", ({ input, idempotencyKey }) => {
  if (!validIdempotency(idempotencyKey)) return { allowed: false, code: "quality.idempotency_required" };
...
registerPolicy("quality.corrective_action.transition", "1", ({ input, idempotencyKey }) => {
  if (!validIdempotency(idempotencyKey)) return { allowed: false, code: "quality.idempotency_required" };
...
registerPolicy("quality.audit.transition", "1", ({ input, idempotencyKey }) => {
  if (!validIdempotency(idempotencyKey)) return { allowed: false, code: "quality.idempotency_required" };
