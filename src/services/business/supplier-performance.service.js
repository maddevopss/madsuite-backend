...
async function createSnapshot({ organisationId, supplierId, periodStart, periodEnd }) {
  // db.query() (contrairement à db.pool.query()) respecte le contexte
  // transactionnel ALS déjà établi par requireOrganisation sur la requête
  // appelante ; db.pool.query() ouvrirait une connexion neuve sans le GUC RLS.
  const supplier = (await db.query('SELECT * FROM suppliers WHERE organisation_id=$1 AND id=$2', [organisationId,supplierId])).rows[0];
  if (!supplier) return null;
  const delivery = (await db.query(`SELECT COUNT(*) FILTER (WHERE received_at::date <= expected_at::date)::numeric on_time,COUNT(*)::numeric total
  ...
  const invoice = (await db.query(`SELECT COUNT(*) FILTER (WHERE matching_status='exception')::numeric exceptions,COUNT(*)::numeric total,COALESCE(SUM(total),0) spend
  ...
  const documents = (await db.query(`SELECT COUNT(*) FILTER (WHERE status='valid' AND (expires_at IS NULL OR expires_at>CURRENT_DATE))::numeric valid,COUNT(*)::numeric total
    FROM supplier_compliance_documents WHERE organisation_id=$1 AND supplier_id=$2`, [organisationId,supplierId])).rows[0];
  const incidents = (await db.query(`SELECT COUNT(*) FILTER (WHERE severity='critical')::numeric critical,COUNT(*)::numeric total
  ...
  const snapshot = (await db.query(`INSERT INTO supplier_performance_snapshots
  ...
  await db.query('UPDATE suppliers SET last_performance_score=$3 WHERE organisation_id=$1 AND id=$2', [organisationId,supplierId,score.overallScore]);
  return snapshot;
}
