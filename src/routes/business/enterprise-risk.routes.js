...
const requireKey = (req, res) => {
  if (!key(req) || String(key(req)).trim().length < 8) {
    res.status(400).json({ code: 'risk.idempotency_required' });
    return false;
  }
  return true;
};
...
// Un risk_id valide pour un autre tenant ne doit pas pouvoir être référencé :
// l'INSERT seul ne vérifie que l'existence de la ligne (FK), pas son organisation.
async function assertRiskInOrg(client, riskId, organisationId) {
  const risk = (await client.query('SELECT id FROM enterprise_risks WHERE id=$1 AND organisation_id=$2', [riskId, organisationId])).rows[0];
  if (!risk) throw notFound('risk.not_found');
}
...
router.post('/controls', (req, res, next) => {
...
router.post('/treatments', (req, res, next) => {
  ...
  return handle(res, next, () => {
    ...
    return transactionalWrite(req, 'risk.treatment.create', policy, { ...input, action: input.status }, async ({ client, organisationId, idempotencyKey }) => {
      await assertRiskInOrg(client, input.riskId, organisationId);
      return (await client.query(`INSERT INTO enterprise_risk_treatments (organisation_id,risk_id,treatment_number,strategy,description,owner_user_id,due_at,evidence,idempotency_key) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`, [organisationId,input.riskId,input.treatmentNumber,input.strategy,input.description,input.ownerUserId,input.dueAt||null,JSON.stringify(input.evidence||[]),idempotencyKey])).rows[0];
    });
  }, 201);
});
...
router.post('/incidents', (req, res, next) => handle(res, next, () => {
  const input = { ...req.body, ownerUserId: req.body.ownerUserId || actor(req) };
  return transactionalWrite(req, 'risk.incident.record', 'risk.incident.record', input, async ({ client, organisationId, idempotencyKey }) => {
    if (input.riskId) await assertRiskInOrg(client, input.riskId, organisationId);
    return (await client.query(`INSERT INTO enterprise_risk_incidents (organisation_id,risk_id,incident_number,source_type,source_id,occurred_at,title,description,severity,impact_summary,evidence,owner_user_id,idempotency_key) VALUES ($1,$2,$3,$4,$5,COALESCE($6,NOW()),$7,$8,$9,$10,$11,$12,$13) RETURNING *`, [organisationId,input.riskId||null,input.incidentNumber,input.sourceType,input.sourceId||null,input.occurredAt||null,input.title,input.description,input.severity||'medium',input.impactSummary||null,JSON.stringify(input.evidence||[]),input.ownerUserId,idempotencyKey])).rows[0];
  });
}, 201));

router.use('/continuity-links', riskContinuityLinksRoutes);
router.use('/risk-links', institutionalRiskLinksRoutes);
