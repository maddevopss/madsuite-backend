...
router.get('/objectives', (req,res,next) => handle(res,next,async () => (await db.query('SELECT * FROM performance_objectives WHERE organisation_id=$1 ORDER BY period_end DESC',[org(req)])).rows));
...
router.get('/indicators', (req,res,next) => handle(res,next,async () => (await db.query('SELECT * FROM performance_indicators WHERE organisation_id=$1 ORDER BY created_at DESC',[org(req)])).rows));
...
router.get('/measurements', (req,res,next) => handle(res,next,async () => (await db.query('SELECT * FROM performance_measurements WHERE organisation_id=$1 ORDER BY measured_at DESC',[org(req)])).rows));
