...
router.post('/timeline', (req, res, next) => handle(res, next, async () => (await db.query(`INSERT INTO resilience_timeline (organisation_id,event_id,entry_type,details,occurred_at,created_by) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`, [org(req), req.body.eventId, req.body.entryType, req.body.details || {}, req.body.occurredAt || new Date(), req.body.createdBy])).rows[0], 201));
...
router.post('/improvements', (req, res, next) => handle(res, next, async () => (await db.query(`INSERT INTO resilience_improvements (organisation_id,lesson_id,title,owner_user_id,due_at,status) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`, [org(req), req.body.lessonId || null, req.body.title, req.body.ownerUserId, req.body.dueAt || null, req.body.status || 'open'])).rows[0], 201));
