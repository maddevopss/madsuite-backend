...
// Un event_id valide pour une autre organisation ne doit pas pouvoir être
// référencé : l'INSERT seul ne vérifie que l'existence de la ligne (FK), pas
// son organisation.
async function assertEventInOrg(client, eventId, organisationId) {
  const event = (await client.query('SELECT id FROM resilience_events WHERE id=$1 AND organisation_id=$2', [eventId, organisationId])).rows[0];
  if (!event) throw notFound('resilience.event_not_found');
}
...
router.post('/crisis-cells', (req, res, next) => handle(res, next, () => transactionalWrite(req, 'resilience.crisis.activate', 'resilience.crisis.activate', { crisisCell: req.body }, async ({ client, organisationId }) => { await assertEventInOrg(client, req.body.eventId, organisationId); return (await client.query(`INSERT INTO resilience_crisis_cells (organisation_id,event_id,lead_user_id,mandate,proof_reference) VALUES ($1,$2,$3,$4,$5) RETURNING *`, [organisationId, req.body.eventId, req.body.leadUserId, req.body.mandate, req.body.proofReference])).rows[0]; }), 201));
...
router.post('/communications', (req, res, next) => handle(res, next, () => transactionalWrite(req, 'resilience.communication.publish', 'resilience.communication.publish', { communication: req.body }, async ({ client, organisationId }) => { await assertEventInOrg(client, req.body.eventId, organisationId); return (await client.query(`INSERT INTO resilience_communications (organisation_id,event_id,author_user_id,approver_user_id,channel,audience,message,published_at,proof_reference,created_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,NOW()) RETURNING *`, [organisationId, req.body.eventId, req.body.authorUserId, req.body.approverUserId, req.body.channel, req.body.audience, req.body.message, req.body.publishedAt || new Date(), req.body.proofReference])).rows[0]; }), 201));
...
router.post('/timeline', (req, res, next) => handle(res, next, async () => {
  await assertEventInOrg(db, req.body.eventId, org(req));
  return (await db.query(`INSERT INTO resilience_timeline (organisation_id,event_id,entry_type,details,occurred_at,created_by) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`, [org(req), req.body.eventId, req.body.entryType, JSON.stringify(req.body.details || {}), req.body.occurredAt || new Date(), actor(req)])).rows[0];
}, 201));
...
router.post('/improvements', (req, res, next) => {
  ...
  return handle(res, next, async () => {
    if (req.body.lessonId) {
      const lesson = (await db.query('SELECT id FROM resilience_lessons WHERE id=$1 AND organisation_id=$2', [req.body.lessonId, org(req)])).rows[0];
      if (!lesson) throw notFound('resilience.lesson_not_found');
    }
