...
const requireKey = (req, res) => { if (!key(req) || String(key(req)).trim().length < 8) { res.status(400).json({ code: 'audit.idempotency_required' }); return false; } return true; };
...
router.post('/findings/:id/close', (req,res,next) => handle(res,next,() => transactionalWrite(req,'audit.finding.close',null,{...req.body,findingId:req.params.id},async ({ client, organisationId, idempotencyKey }) => {
  ...
  return (await client.query(`UPDATE internal_audit_findings SET status='closed',closure_reason=$1,evidence=$2,updated_at=NOW() WHERE id=$3 AND organisation_id=$4 RETURNING *`,[req.body.closureReason,JSON.stringify(req.body.evidence||[]),req.params.id,organisationId])).rows[0];
})));
