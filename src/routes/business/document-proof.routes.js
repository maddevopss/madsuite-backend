...
router.get("/records", (req, res, next) => handle(res, next, async () => (await db.pool.query(
...
router.get("/records/:id/versions", (req, res, next) => handle(res, next, async () => (await db.pool.query(
...
router.get("/links/:aggregateType/:aggregateId", (req, res, next) => handle(res, next, async () => (await db.pool.query(
...
router.post("/records/:id/links", (req, res, next) => handle(res, next, async () => (await db.pool.query(
...
router.get("/alerts", (req, res, next) => handle(res, next, async () => {
  ...
  const [retention, unversioned] = await Promise.all([
    db.pool.query(`SELECT id,document_number,title,retention_until,legal_hold FROM document_records WHERE organisation_id=$1 AND retention_until<=CURRENT_DATE+INTERVAL '60 days' ORDER BY retention_until`, [organisationId]),
    db.pool.query(`SELECT id,document_number,title FROM document_records WHERE organisation_id=$1 AND current_version_id IS NULL ORDER BY created_at`, [organisationId]),
  ]);
