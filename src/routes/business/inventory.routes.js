    ...
    const { rows } = await req.db.query(
      `SELECT i.*,
              ...
              CASE WHEN COALESCE(SUM(b.quantity),0) > 0
                   THEN (COALESCE(SUM(b.inventory_value),0) / SUM(b.quantity))::numeric
...
router.post("/items", requireRole("admin"), async (req, res, next) => {
  try {
    const body = req.body;
    if (!String(body.sku || "").trim() || !String(body.name || "").trim()) {
      return res.status(400).json({ message: "Le SKU et le nom sont obligatoires." });
    }
    const { rows } = await req.db.query(
...
router.post("/locations", requireRole("admin"), async (req, res, next) => {
  try {
    if (!String(req.body.code || "").trim() || !String(req.body.name || "").trim()) {
      return res.status(400).json({ message: "Le code et le nom de l’emplacement sont obligatoires." });
    }
    const { rows } = await req.db.query(
...
router.post("/movements", requireRole("admin"), async (req, res, next) => {
  const mapping = { receipt: "receipt", issue: "issue", adjustment: "adjustment", transfer: "transfer" };
  const type = mapping[req.body.movementType];
    ...
    const { rows } = await req.db.query(
      `SELECT t.*, i.sku, i.name item_name, sl.name source_location_name, dl.name destination_location_name
       ...
       WHERE t.organisation_id=$1
         AND ($2::bigint IS NULL OR t.item_id=$2)
