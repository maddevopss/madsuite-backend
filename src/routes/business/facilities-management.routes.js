...
const requireKey = (req, res) => { if (!idempotency(req) || String(idempotency(req)).trim().length < 8) { res.status(400).json({ code: 'facilities.idempotency_required' }); return false; } return true; };
...
router.post('/maintenance-links', (req, res, next) => {
  if (!requireKey(req, res)) return;
  return handle(res, next, () => transactionalWrite(req, 'facilities.maintenance_link.create', null, req.body, async ({ client, organisationId, idempotencyKey }) => {
