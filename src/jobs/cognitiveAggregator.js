...
async function aggregateOrganisation(client, organisationId, targetDateStr) {
...
async function aggregateCognitiveMetrics() {
    ...
    try {
        ...
        // organisations n'a pas de colonne deleted_at (pas de soft-delete) : ce
        // filtre faisait échouer la requête à chaque exécution, empêchant le job
        // de démarrer.
        const orgsRes = await db.query(`SELECT id FROM organisations`);
        let totalUsersAggregated = 0;

        for (const org of orgsRes.rows) {
            const organisationId = org.id;

            // cognitive_state_events/daily_cognitive_metrics sont sous RLS FORCE :
            // ce job en lot traite toutes les organisations hors de tout contexte
            // requête — sans ce scoping explicite, aucune ligne n'est visible ni
            // écrivable et le job agrège silencieusement zéro utilisateur, chaque nuit.
            const client = await db.pool.connect();
            try {
                await client.query('BEGIN');
                await client.query("SELECT set_config('app.current_organisation_id', $1, true)", [String(organisationId)]);
                const usersAggregated = await dbStore.run(
                    { dbClient: client, organisationId },
                    () => aggregateOrganisation(client, organisationId, targetDateStr),
                );
                await client.query('COMMIT');
                totalUsersAggregated += usersAggregated;
            } catch (error) {
                try { await client.query('ROLLBACK'); } catch (_) {}
                logger.error(`[CognitiveAggregator] Erreur pour l'organisation ${organisationId}:`, { error: error.message });
            } finally {
                client.release();
            }
        }
