-- DANGER: manual snapshot reset. Never use for schema upgrades.
-- The source of truth is ../migrations/.
-- ============================================================
-- MADSuite / TimeMonitoring
-- db.sql complet organisé
-- ============================================================

-- DANGER:
-- This file is destructive. It drops and recreates the "madsuite" database.
-- Never run it against a database that contains data you want to keep.
-- To update an existing database, use backend/db/migrations/ instead.

-- ATTENTION :
-- DROP DATABASE / CREATE DATABASE doivent être lancés depuis une DB admin.
-- Si tu es déjà connecté à madsuite, utilise plutôt les fichiers migrations/.
...
-- Après création : connecte-toi à madsuite, puis exécute schema/index/seed.
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_one_active_timer_per_user ON time_entries(utilisateur_id) WHERE end_time IS NULL AND deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_invoices_org_status ON invoices(organisation_id, status) WHERE deleted_at IS NULL;
